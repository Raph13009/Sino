...
The contact form on `/contact` (and `/zh/contact`) submits on the OPOPA site through a Next.js Server Action. Visitors are never redirected to a third-party page. Email is sent with [Resend](https://resend.com).
...
Until a domain is verified, Resend only allows sending from `beth.t@example.com`. This implementation uses that address by default so mail can be tested without claiming `contact@opopa-partners.com`.
...
## Verify the future OPOPA domain

When the production domain is ready:

1. In Resend, open **Domains** and add `opopa-partners.com`.
2. Add the DNS records Resend shows (SPF, DKIM, and any MX/verification records).
3. Wait until the domain status is verified.
4. Set the sender:

```bash
RESEND_FROM_EMAIL=OPOPA <contact@opopa-partners.com>
```

Add the same variable in Vercel, then redeploy.

Do not set `RESEND_FROM_EMAIL` to `contact@opopa-partners.com` before the domain is verified. Resend will reject unverified senders.
...
`New OPOPA enquiry — [Company name]`
...
6. Confirm:
   ...
   - the notification email arrives at the OPOPA inbox
...
5. Confirm the visitor is not redirected off the OPOPA domain.
